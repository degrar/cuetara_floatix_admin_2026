...
<template>
    <div class="relative">
        ...
        <div class="">
            <div class="relative bg-white text-grey font-montserrat rem:text-[14px] rem:leading-[16px] std-input w-full rounded-[6px] appearance-none px-2 py-3 rem:h-[45px] rem:mt-[5px] rem:mb-[5px] border-[1px] border-softGrey placeholder-softGrey placeholder-opacity-100 focus:placeholder-opacity-50 focus:outline-none focus:border-softGrey focus:shadow-none" :class="{ '!border-error text-error': error }">
            ...
            <div v-if="error" class="rem:text-[12px] font-bold font-montserrat text-error inline-block">{{ error }}</div>
        </div>
    </div>
</template>
